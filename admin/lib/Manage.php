...
class Manage extends DB
{
     ...
     * @return bool
     */
    public function update( $catname, $cattplid, $listfile = '', $listtpl = '', $filetpl = '',
    ...
    public function del($id)
    {
        //删除目录ID
         function getallid($mana,$id,$r,$tmpr=array()){
            array_push($r,$id);
            $tmpr=array_merge_recursive($tmpr,$mana->query(0,$id));
            if(empty($tmpr)){
                return $r;
            }
        ...
        $sql = sprintf('DELETE FROM `f_system_manage` WHERE `f_system_manage`.`id` IN(%s)', implode(',',$allid));
        $r = $this->pdo->exec($sql);
        $this->errorinfo = $this->pdo->errorInfo();
        if ($r === false) {
             return false;
     ...
     * @return Array
     */
    public function query($id=0,$pid=0)
    {
        if($pid>0){
            $sql = sprintf('SELECT * FROM `f_system_manage` WHERE `f_system_manage`.`parent_catalog` = %d',$pid);
        }else if($id===0){
